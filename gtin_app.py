...
def decide_gender(parsed, women_max=245):
    """성별 자동 결정.
    1) 옵션명에 '여성/남성' 명시가 있으면 그대로.
    2) 없으면 사이즈 기준: women_max 이하면 여성, 초과면 남성.
       (통상 230~245 여성 / 250~280 남성)
    3) 사이즈를 모르면 남성 기본.
    """
    if parsed["gender"] in ("여성", "남성"):
        return parsed["gender"]
    size = parsed["size"]
    if size and str(size).isdigit():
        return "여성" if int(size) <= women_max else "남성"
    return "남성"


# ---------------------------------------------------------------------------
# detailinfo 읽기: 등록상품명(2), 등록옵션명(13), 바코드(231).
#   상품명은 첫행만 채워지고 이후 빈칸은 직전 상품 소속.
#   바코드가 있는 행 = 쿠팡이 로켓배송용으로 발급한 것(로켓/일반 중복의 로켓쪽).
# ---------------------------------------------------------------------------
BARCODE_COL = 231
...
def read_detailinfo(file_bytes):
    ...
    for r in range(5, ws.max_row + 1):
        ...
        rows.append({
            "product": cur_name,
            "option": str(opt).strip() if opt is not None else "",
            "barcode": str(bc).strip() if bc is not None else "",
        })
    return rows


def dedupe_rows(rows, mode="general"):
    """로켓/일반 중복 제거 + 생성 대상 선택.

    같은 (상품명, 옵션명=색상+사이즈) 조합은 항상 1개로 합친다.
    mode:
      - "general"  : 바코드 없는 '일반' 행 우선. 일반 없이 쿠팡발급만 있는
                     조합은 제외. (기본)
      - "all"      : 바코드 유무 상관없이 모든 옵션 생성(중복만 1개로).
                     일반 행이 있으면 일반을, 없으면 쿠팡발급 행을 대표로.
      - "issued"   : 이미 바코드(쿠팡발급)가 있는 옵션만.
    반환: (유지행 리스트, 통계 dict)
    """
    from collections import defaultdict
    g = defaultdict(list)
    for i, r in enumerate(rows):
        g[(r["product"], r["option"])].append(i)

    kept = []
    removed = 0
    excluded = 0
    for key, idxs in g.items():
        no_bc = [i for i in idxs if not rows[i]["barcode"]]
        has_bc = [i for i in idxs if rows[i]["barcode"]]

        if mode == "issued":
            if has_bc:
                chosen = dict(rows[has_bc[0]])
                chosen["coupang_only"] = (len(no_bc) == 0)
                kept.append(chosen)
                removed += len(idxs) - 1
            else:
                excluded += len(idxs)        # 발급분 없음 → 전부 제외
            continue

        # general / all 공통: 일반 우선, 없으면 발급분 대표
        if no_bc:
            chosen = dict(rows[no_bc[0]])
            chosen["coupang_only"] = False
            kept.append(chosen)
            removed += len(idxs) - 1
        else:
            removed += len(idxs) - 1
            if mode == "all":
                chosen = dict(rows[has_bc[0]])
                chosen["coupang_only"] = True
                kept.append(chosen)
            else:  # general → 일반 없는 조합은 제외
                excluded += 1
...
with st.sidebar:
    ...
    st.caption("여성용·남성용 기본 분류를 각각 지정하세요. "
               "표에서 행별로 여성/남성을 토글하면 해당 분류가 적용됩니다.")

    all_labels = list(CLASS_LABEL2CODE.keys())

    def _class_picker(title, kw_default, prefer):
        """검색창 + 드롭다운으로 분류 1개 선택. prefer 문자열을 포함하는
        항목을 기본 인덱스로."""
        q = st.text_input(f"{title} 검색", value=kw_default, key=f"q_{title}")
        if q.strip():
            kws = q.strip().split()
            opts = [lbl for lbl in all_labels
        ...
        for i, lbl in enumerate(opts):
            if prefer and prefer in lbl:
                idx = i
                break
        sel = st.selectbox(f"{title} 분류", opts, index=idx, key=f"sel_{title}")
        return CLASS_LABEL2CODE[sel]

    women_class = _class_picker("여성", "여성 운동화", "여성신발 > 운동화 > 러닝화")
    men_class = _class_picker("남성", "남성 운동화", "남성신발 > 운동화 > 러닝화")

    st.caption("자동 성별 판단 기준 (옵션명에 여성/남성 명시가 없을 때)")
    women_max = st.number_input(
        "이 사이즈 이하 = 여성, 초과 = 남성", value=245, step=5,
        help="통상 230~245 여성 / 250~280 남성. 명시된 성별이 우선합니다.")

    CODE2LABEL = {v: k for k, v in CLASS_LABEL2CODE.items()}
    class_ready = bool(women_class and men_class)
...
if up is not None and class_ready:
    raw_rows = read_detailinfo(up.getvalue())
    ...
    mode_label = st.radio(
        ...
        ["일반만 (바코드 없는 미발급분, 권장)",
         ...
         "이미 발급된 것만 (바코드 있는 옵션)"],
        index=0,
        help="로켓/일반 중복은 어느 모드든 색상·사이즈별 1개로 합쳐집니다.")
    mode = {"일반만 (바코드 없는 미발급분, 권장)": "general",
            "전부 생성 (이미 바코드 있어도 생성)": "all",
            "이미 발급된 것만 (바코드 있는 옵션)": "issued"}[mode_label]

    rows, dstats = dedupe_rows(raw_rows, mode=mode)

    st.success(
        f"원본 {dstats['원본행']:,}행 → **{dstats['유지행']:,}행** 생성 대상 "
        f"(중복 {dstats['중복제거']:,}행 정리)")
    if dstats["제외"]:
        st.info(f"선택한 모드 기준 제외된 옵션 {dstats['제외']:,}개")
    ...
    # 분류 라벨(여성/남성 기본분류만 선택지로) + 직접지정용 전체
    ...
    with st.expander("최종 데이터 미리보기"):
        st.dataframe(
            final[["상품명", "옵션명", "성별", "상품분류코드", "제조사명", "브랜드명",
