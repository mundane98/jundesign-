...
REF = load_ref()
CLASS_LIST = REF["class"]                      # [{code, name}]
COLOR_LIST = REF["color"]                       # [{code, name(영문)}]
SHOE_LIST = REF["shoe_size"]                    # [{code, size}]

# 분류: 표시라벨 -> 코드
CLASS_LABEL2CODE = {f'{c["code"]}  |  {c["name"]}': c["code"] for c in CLASS_LIST}
# 색상: 영문명 -> 코드
...
# ---------------------------------------------------------------------------
# 결과 엑셀 생성 (업로드 템플릿 시트 형식)
# ---------------------------------------------------------------------------
TEMPLATE_HEADERS = [
    "순번", "상품분류", "제조사명", "브랜드명", "상세 상품명",
    "내용량", "내용량 단위", "주요판매국가", "가로(cm)", "세로(cm)",
    "높이(cm)", "총중량(g)", "제조국/생산국", "과세형태", "수입여부",
    "모델명", "KC인증정보", "(R)", "(S)", "사용안함",
    "색상코드", "사이즈코드", "하위브랜드명", "영문상세상품명",
    "상품이미지URL", "출시일", "단종일",
]

def build_output_xlsx(df, fixed):
...
with st.sidebar:
    ...
    st.divider()
    st.header("2. 상품분류 선택")
    q = st.text_input("분류 검색(키워드)", placeholder="예: 운동화, 신발, 슈즈")
    if q.strip():
        kws = q.strip().split()
    ...
    if filtered:
        sel_class = st.selectbox("상품분류", filtered[:500],
                                 help="검색으로 좁힌 뒤 선택")
...
if up is not None and class_code:
    ...
    st.subheader("6. 다운로드")
    out_bytes = build_output_xlsx(final, fixed)
    st.download_button(
        "📥 GTIN 템플릿(.xlsx) 다운로드",
        data=out_bytes,
